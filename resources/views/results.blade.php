...
<div class="container column is-8">
	<section class="section">
		...
		<div class="box has-background-info">
			<h1 class="title" id="titles"><a class="has-text-white-ter" href="{{ route('show_molecule', [ 'id'=> $vf->id ]) }}">{{$vf->Designation}}</a></h1>
		</div>
		@endforeach @foreach($lineaments as $vf)
		<div class="box has-background-primary">
			<h1 class="title" id="titles"><a class="has-text-white-ter" href="{{ route('show_lineament', [ 'id'=> $vf->id ]) }}">{{$vf->Designation}}</a></h1>
		</div>
		@endforeach
	</section>
</div>
